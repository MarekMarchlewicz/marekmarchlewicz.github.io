var ItemManager = new function(){
	...
	this.AwardReward = function(reward,qty,callback){
        UniwebviewManager.AwardReward(parseInt(reward),qty,callback);
    }
    this.AwardPersistentReward=function(reward,qty,msg,callback){
        UniwebviewManager.AwardPersistentReward(reward,qty,msg,callback);
    }
    this.resetPersistentReward = function (reward,callback){
        ItemManager.cb = callback;
        UniwebviewManager.ResetPersistentReward(reward,"ItemManager.callback");
    }
    this.AwardMultiplayerReward=function(reward,qty,msg,callback){
        UniwebviewManager.AwardMultiplayerReward(reward,qty,msg,callback); 
    }
    ...
    this.getPersistentItems = function(callback){
        ItemManager.cb = callback;
        UniwebviewManager.GetPersistentInventory("ItemManager.getPersistentCallback");
    }
    this.getMinigameRewards = function(rewardObj,callback){
        if(rewardObj.type=="reward"){
            UniwebviewManager.AwardReward(parseInt(rewardObj.id),1,callback);
        }
        else if(rewardObj.type=="persistentreward"){
            UniwebviewManager.AwardPersistentReward(rewardObj.id,1,"",callback);
        }
        else if(rewardObj.type=="multiplayerreward")UniwebviewManager.AwardMultiplayerReward(rewardObj.id,1,"",callback); 
    }
    this.message="";

...
var DialogRunner = new function()
{
    ...
    this.stopAudio=function(){
        UniwebviewManager.StopDialog("DialogRunner.playGesture");
    }
    this.playGesture = function(){
		if(gestureObj.gesture!=undefined){
            UniwebviewManager.PlayGesture(gestureObj.gesture,gestureObj.targetChar,gestureObj.gestureLoop,"DialogRunner.playEmotion");
        }
        else DialogRunner.playEmotion();
    }
	this.playEmotion=function(){
        if(emotionObj.emotion!=undefined){
            UniwebviewManager.PlayEmotions(gestureObj.emotion,gestureObj.targetChar,"DialogRunner.afterPlayEmotions");
        }
    ...
    this.parseDialogObject = function(){
        ...
        if(gesture!="" && gesture!=null){
            if(gestureTiming==0) UniwebviewManager.PlayGesture(gesture, targetChar, gestureLoop,"DialogRunner.playEmotions");
                ...
                UniwebviewManager.PlayGesture(gesture, targetChar, gestureLoop,"DialogRunner.playEmotions");
            },gestureTime*1000);
        }
    ...
    this.stopNPCAnimation=function(){
        if(stopGesture==true&&stopGesture!=""){
            if(DialogRunner.dialogobject.NpcInstanceID != null)UniwebviewManager.ResetGestureNEmotion(DialogRunner.dialogobject.NpcInstanceID,"DialogRunner.stopPlayerAnimation");
    ...
    this.stopPlayerAnimation=function(){
        if(stopGesture==true&&stopGesture!=""){
            if(Global.playerAvatarInstanceID != null)UniwebviewManager.ResetGestureNEmotion(Global.playerAvatarInstanceID,"Dialog.close");
            else Dialog.close();
        }
        else Dialog.close();
        reward="";
    ...
    this.playEmotions =function(){
        ...
        if(emotion!="" && emotion!=null){
            if(emotionTiming==0)UniwebviewManager.PlayEmotions(emotion, targetChar,"DialogRunner.playCameraEffects");
                ...
                UniwebviewManager.PlayEmotions(emotion, targetChar,"DialogRunner.playCameraEffects");
            },emotionTime*1000); 
        }
    ...
    this.playCameraEffects=function(){
        ...
        if(cameraEffect==1||cameraEffect==3){
            ...
            cameraBindingFunction="CameraUniWebViewBinder.FadeOut";
            cameraFadeDuration = parseFloat($(Global.tmp[DialogRunner.childindex]).attr("cameraFadeDuration"));
        }
        else if(cameraEffect==6 || cameraEffect==7){
            if(cameraEffect==6)cameraBindingFunction="CameraUniWebViewBinder.ShakeCamera";
            else cameraBindingFunction="CameraUniWebViewBinder.ShakeRotateCamera";
        ...
        if(cameraEffect!=""&&cameraEffect!=0&&cameraEffect!=1&&cameraEffect!=3){
            if(cameraEffect==2){
                ...
                UniwebviewManager.setCameraEffects(cameraBindingFunction,[cameraShakeDuration,cameraShakeMagnitude],"DialogRunner.playCameraSettings");
            }
    ...
    this.playCameraSettings=function(){
        ...
        if(shotType!=null && shotType!="")UniwebviewManager.SwitchToMachinima(shotType,cameraPanState,targetChar,notTargetChar,cameraPosition,cameraRange,cameraHeight,cameraDuration,cameraOffsetX,cameraOffsetY,cameraOffsetZ,"DialogRunner.parseDialogandShow");
    ...
    this.playAudio=function(){
        ...
        if(audiofile != "")UniwebviewManager.PlayDialog(audiofile,"DialogRunner.addClick");
...
var Dialog = new function()
{
	...
	this.initSkeleton = function(callback)
	{
		...
		UniwebviewManager.GetPlayerInstanceIDAndName("Dialog.GetPlayerInstanceIDAndName");
		this.firstrun = false;
	}
    this.callback=null;
	...
	this.close = function(){
        ...
        UniwebviewManager.StopDialog("Dialog.EndInteraction");
	}
    ...
    this.EndInteraction=function(data){
        ...
        if(TriggerManager.ar.length>0){
            Inventory.getInventoryJSON(TriggerManager.ar,"UniwebviewManager.EndInteraction");
        }
        else UniwebviewManager.EndInteraction();
        TriggerManager.ar = [];
    }
    ...
    this.tempClose = function(){
		...
		//IF ANY SOUND
		UniwebviewManager.StopDialog("Dialog.playMinigame");
    }
